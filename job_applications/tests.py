...
class JobApplicationViewAPITests(APITestCase):
    ...
    def test_job_application_create_view(self):
        ...
        response = self.client.post('/api/job-applications/create', data, format='json')
        print(response)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(JobApplication.objects.count(), 1)
        self.assertEqual(JobApplication.objects.get().status, 'Applied')

    #TODO: Revisit the ListView and UpdateView tests
    # def test_job_application_list_view(self):
    #     # Retrieve the list of JobApplications
    #     response = self.client.get('/api/job-applications/')
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
    #     print(JobApplication.objects.count())
    #     print(response)
    #     print(response.data)
    ...
    #     response = self.client.put(f'/api/job-applications/{job_application.id}/', data, format='json')
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
